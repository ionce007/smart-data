...
/**
 * 解析字符串并判断是否为布尔类型（完善空值/边界值处理）
 * @param {*} str 要解析的值（支持任意类型，包括null/undefined）
 * @returns {Object} { isBoolean: 布尔值, value: 解析后的布尔值/undefined }
 * @description 优先处理null/undefined，避免转为字符串后误判
 */
export function parseBoolean(str) {
    // 1. 空值判断
    if (str === null || str === undefined) {
        return { isBoolean: false, value: undefined };
    }
...
/**
 * 递归的方法，按指定的key、value查找对象
 * @param {*} obj 待查的对象
 * @param {*} targetKey 指定的Key
 * @param {*} value 指定的Key对应的值
 * @returns 查找到的对象，若未查找到，则返回null
 */
export function findValue(obj, targetKey, value) {
    if (typeof obj !== 'object' || obj === null) return null;

    if (obj.hasOwnProperty(targetKey)) {
        if (obj[targetKey] === value) return obj;
    }

    for (let key in obj) {
        if (typeof obj[key] === 'object') {
            const result = findValue(obj[key], targetKey, value);
